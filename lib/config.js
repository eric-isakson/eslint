        ...
        Object.keys(loadedPlugins).forEach(function(pluginNameWithoutPrefix) {
            ...
            if (plugin.environments) {
                Object.keys(plugin.environments).forEach(function(environmentName) {
                    var environment = plugin.environments[environmentName],
                        rulesWithPluginName = {};
                    // replace */ in rule definitions with pluginname/
                    if (environment.rules) {
                        Object.keys(environment.rules).forEach(function(ruleName) {
                            var ruleWithPluginName = ruleName.replace(/^\*\//, pluginNameWithoutPrefix + "/");
                            rulesWithPluginName[ruleWithPluginName] = environment.rules[ruleName];
                        });
                        environment.rules = rulesWithPluginName;
                    }
